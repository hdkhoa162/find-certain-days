...
const findCertainDates = (date) => {
    ...
    if (!weekDays.includes(date)) {
        throw new Error('please enter the weekday properly such as: Monday, Tuesday....');
    }
    ...
    while (currentMonthIdx <= monthArrLength) {
        result = result.concat(findCertainDateInMonths(date, (currentMonthIdx + 2), year));
        currentMonthIdx++;
    }
...
const findCertainDateInMonths= (weekDay, nextMonthIdx, year) => {

    const result = [];
    const weekDays = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    const beginOfMonth = moment([year, nextMonthIdx], 'YYYY-MM-DD');
